(function($) {
	...
	$(document).ready(function() {
		...
		let tutorial_button = $('#start-introjs');
		let integration = tutorial_button.data('integration');
		let steps = [];
		...
		if (!integration) {
			...
			if ($('#bugs-tabs-content .table .upload_bug').length) {
				if (integration === "csv_exporter") {
					steps.push(	{
						...
						intro: _x("Click here to export a specific bug","Integration Center Introjs", "integration-center")
					})
				} else {
					steps.push(	{
						element: $('#bugs-tabs-content .table .upload_bug')[0],
			...
			if (integration === "csv_exporter") {
				steps.push(	{
					element: $('.send-all')[0],
					intro: _x("You can export all the list","Integration Center Introjs", "integration-center")
				})
				steps.push(	{
					element: $('.send-selected')[0],
					intro: _x("You can export all the selected bugs","Integration Center Introjs", "integration-center")
				})
			} else {
				steps.push(	{
					element: $('.send-all')[0],
			...
			if ($('#settings .settings-wizard').length) {
				...
				if (integration === "csv_exporter") {
					steps.push(	{
						...
						intro: _x("Edit settings, here you can also select the file format and the fields for the export","Integration Center Introjs", "integration-center")
					})
				} else {
					steps.push(	{
						element: $('[data-target="#custom_tracker_settings_modal"]')[0],
		...
		const startTutorial = function startTutorial() {
			tutorial.start();
		};
